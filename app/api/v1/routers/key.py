...
async def save_api_token(api_token: APIKeyCreate, user: CurrentActiveVerifiedUserDep, uow: AsyncUOWDep):
...
async def get_api_tokens(user: CurrentActiveVerifiedUserDep, uow: AsyncUOWDep):
    return await ProfileService.get_api_tokens(session=uow, user=user, trim=True)


@keys_router.delete('/delete/{key_id}')
async def delete_api_token(key_id: uuid.UUID, user: CurrentActiveVerifiedUserDep, uow: AsyncUOWDep):
